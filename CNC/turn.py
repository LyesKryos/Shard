...
class Turn:
    ...
    async def _user_updates(self):
        ...
        for user in user_list:
            ...
            military_alliances = await conn.fetchval('''SELECT cardinality(members) FROM cnc_military_alliances 
            ...
            trade_value = min(trade_good_production * trade_good_production_access,0)
    ...
    async def _timer_updates(self):
        ...
        # reduce all by one
        await conn.execute('''UPDATE cnc_users
                              SET govt_type_countdown = CASE 
                                                            WHEN govt_type_countdown > 0 
                                                                THEN govt_type_countdown = govt_type_countdown - 1 END,
                                  govt_subtype_countdown = CASE 
                                                               WHEN govt_subtype_countdown > 0 
                                                                   THEN govt_subtype_countdown - 1 END;''')
