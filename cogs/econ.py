    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "One Week Loan":
            ...
            if self.amount < 10000:
                return await interaction.followup.edit(f"Two week loans must be more than {self.thaler}10,000.")
        elif self.values[0] == "Three Week Loan":
            interest = 6
            due = self.three_weeks
            if self.amount < 50000:
                return await interaction.followup.edit(f"Three week loans must be more than {self.thaler}50,000.")
        else:
            interest = 4
            due = self.month
            if self.amount < 100000:
                return await interaction.followup.edit(f"Month loans must be more than {self.thaler}100,000.")
        # establish connection
        conn = interaction.client.pool
