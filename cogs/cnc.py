...
class MapButtons(View):
    ...
    async def main_map(self, interaction: discord.Interaction, main: discord.Button):
        ...
        await self.message.edit(content="https://i.ibb.co/Qj6b0sVs/Terrain-Map.png", attachments=[])
    ...
    async def terrain_map(self, interaction: discord.Interaction, terrain: discord.Button):
        # defer the interaction because otherwise stuff crashes
        await interaction.response.defer()
        await self.message.edit(content="https://i.ibb.co/wZ4FzJYQ/Terrain-Only.png", attachments=[])
    ...
    async def carto_map(self, interaction: discord.Interaction, carto: discord.Button):
        # defer the interaction because otherwise stuff crashes
        await interaction.response.defer()
        await self.message.edit(content="https://i.ibb.co/x8j56fxy/Geography-Map.png", attachments=[])
...
class DossierView(View):
    ...
    async def economy(self, interaction: discord.Interaction, button: discord.Button):
        ...
        average_development = await self.conn.fetchval('''SELECT avg(development) FROM cnc_provinces 
                                                          WHERE owner_id = $1;''', self.user_info['user_id'])
        trade_goods_produced = await self.conn.fetchval('''SELECT array_agg(DISTINCT trade_good) 
                                                           FROM cnc_provinces WHERE owner_id = $1;''', self.user_info['user_id'])
        ...
        self.doss_embed.add_field(name="Trade Goods Produced", value=f"{', '.join(trade_goods_produced)}", inline=False)
        # update
        await interaction.edit_original_response(embed=self.doss_embed)
        ...
        if user_info['name'] in war_info['attackers']:
            ...
            if len(defenders_list) > 1:
                ...
                async def cancel_button_callback(interaction: discord.Interaction):
                    await interaction.response.defer()
                    for child in peace_target_view.children:
                        child.disabled = True
                    return await self.interaction.edit_original_response(view=peace_target_view)
...
class ArmyActionsView(View):
    ...
    async def attach_army(self, interaction: discord.Interaction):
        ...
        if not army_list:
            ...
            await self.parent_interaction.edit_original_response(view=self)
            # reject
            return await interaction.response.send_message("There are no eligible armies to attach to at this location.",
                                                           ephemeral=True)
        ...
        # if the option was recruit, attempt to recruit a new general
            ...
            # otherwise, carry on
                ...
                await conn.execute('''INSERT INTO cnc_generals(owner_id, type, level, army_id, name) 
                                   ...
                                   choice(['Assault', 'Defensive', 'Assault', 'Defensive', 'Siege']),
                                   user_info['gen_level'],
...
class CommandAndConquest(commands.Cog):
    ...
    async def cog_load(self):
        ...
        # check if the loop is running and post
        if self.turn_loop.is_running():
    ...
    async def move_army(self, interaction: discord.Interaction, army: int, move_to: str):
        ...
        if army_info['owner_id'] != user_info['user_id']:
            return await interaction.followup.send(
                f"{user_info['name']} does not command the {army_info['army_name']}.",
                ephemeral=True)
        # if the army is attached
        if army_info['attached'] is not None:
            # reject
            return await interaction.followup.send(f"The {army_info['army_name']} is attached to another army "
                                                   f"and cannot move independently.",
                                                   ephemeral=True)
        ...
        async def get_enemy_ids(war: asyncpg.Record, user_name: str) -> List[int]:
            # check for the enemy names
            enemy_names = war['defenders'] if user_name in war['attackers'] else war['attackers']
            # define the ids
            ids = []
            # cycle through to get the ids
            for name in enemy_names:
                uid = await conn.fetchval('SELECT user_id FROM cnc_users WHERE name = $1;', name)
                ids.append(uid)
            return ids
        ...
        if army_info['embark']:
            ...
            if "Water Transportation" in user_info['tech']:
                ...
                movement_cost *= .9
            # if the army does not have enough movement
            ...
            # if the attached armies don't have enough movement
            if attached_armies and attached_total_movement < movement_cost:
                return await interaction.followup.send(
                    f"The armies attached to {army_info['army_name']} does not have sufficient movement to sail to "
                    f"{prov_info['name']} (ID: {prov_info['id']}).", ephemeral=True)

            # check access if province is occupied by someone else
            if prov_info['occupier_id'] not in (0, user_info['user_id']):
                # pull the occupier info
                occupier_info = await user_db_info(conn=conn, user_id=prov_info['occupier_id'])
                # check for wars and mil access
                war_check = await conn.fetchrow('''SELECT *
                                                   FROM cnc_wars
                                                   WHERE (($1 = ANY (attackers) AND $2 = ANY (defenders))
                                                       OR ($1 = ANY (defenders) AND $2 = ANY (attackers)))
                ...
                mil_access_check = await conn.fetchrow('''SELECT *
                                                          ...
                                                          WHERE $1 = ANY (members)
                                                            AND $2 = ANY (members);''',
                                                       user_info['name'], occupier_info['name'])
                # if the army does not have access
                if not war_check and not mil_access_check:
                    return await interaction.followup.send(
                        f"The {army_info['army_name']} is not authorized to land at "
                        f"{prov_info['name']} (ID: {prov_info['id']}).", ephemeral=True)
                # otherwise, define the global war as the war check
                if war_check:
                    war = war_check

            # if native province, check war and province count
            if prov_info['occupier_id'] == 0:
                # pull war
                war = await conn.fetchrow('''SELECT *
                                             FROM cnc_wars
                                             WHERE ($1 = ANY (attackers) OR $1 = ANY (defenders))
                                               AND active = True;''',
                                          user_info['name'])
            ...
            if war is not None and not battle:
                # check the user side
                user_side = 'attacker' if user_info['name'] in war['attackers'] else 'defender'
                # get the enemy ids
                enemy_ids = await get_enemy_ids(war, user_info['name'])
                ...
                # otherwise, occupy
                    ...
                    return await interaction.followup.send(
                        ...
                        f"(ID: {prov_info['id']}), adding `{round(score)}` War Score to the {war['name']}.")
        ...
        # if they are moving by land
        else:
            # calculate the path
            path, cost, blocked_by_hostiles = await find_path(
                conn=conn, start_id=army_info['location'], end_id=move_to, moving_user_id=interaction.user.id)

            # path validation
            if path is None:
                # if the movement is blocked by hostiles, send a special message
                if blocked_by_hostiles:
                    return await interaction.followup.send(
                        f"The {army_info['army_name']} is unable to reach {prov_info['name']} (ID: {prov_info['id']})."
                        f" The path is blocked by hostile armies, enemy forts, or lack of military access.",
                        ephemeral=True)
                # otherwise, normal message
                return await interaction.followup.send(
                    f"The {army_info['army_name']} cannot find an open land path to "
                    f"{prov_info['name']} (ID: {prov_info['id']}).",
                    ephemeral=True)

            # if the army has insufficient movement
            if cost > army_info['movement']:
                return await interaction.followup.send(
                        f"The {army_info['army_name']} cannot reach {prov_info['name']} (ID: {move_to}) because of "
                        f"insufficient movement.\nCost from {depart_prov_info['name']} "
                        f"(ID: {depart_prov_info['id']}): `{cost}` points.")
            # if the attached armies have insufficient movement
            if attached_armies and attached_total_movement < cost:
                return await interaction.followup.send(
                    f"The armies attached to {army_info['army_name']} cannot reach {prov_info['name']} "
                    f"(ID: {move_to}) because of insufficient movement.\nCost from {depart_prov_info['name']} "
                    f"(ID: {depart_prov_info['id']}): `{cost}` points.")

            # if the province owner is natives
            if prov_info['owner_id'] == 0:
            ...
            # if the province is occupied by neither natives nor the user
            elif prov_info['occupier_id'] not in (0, user_info['user_id']):
                # get the occupier info
                occupier_info = await user_db_info(conn=conn, user_id=prov_info['occupier_id'])
                # check for wars
                war_check = await conn.fetchrow('''SELECT *
        ...
        if battle:
            ...
            if prov_info['owner_id'] == 0 and len(enemy_army_list) == 0:
                ...
                # === DEFEAT ===
                    ...
                    # get troop totals
                    troop_total = army_info['troops'] + (sum(a['troops'] for a in attached_armies) if attached_armies else 0)
                    # check if the attacker army was destroyed or should be destroyed
                    if attacker_casualties >= (.8 * troop_total):
                        # destroy the army
                        await conn.execute('''DELETE FROM cnc_armies WHERE army_id = $1 or attached = $1;''',
                                           army_info['army_id'])
                        # unassign any generals
                        await conn.execute('''UPDATE cnc_generals SET army_id = NULL 
                                              WHERE army_id = $1 OR attached = $1;''',
                                           army_info['army_id'])
                ...
                # get the attacking army info
                general_name = await conn.fetchval('''SELECT name FROM cnc_generals WHERE army_id = $1;''', army_info['army_id'])
                attacking_armies = army_info['army_name'] + (", " if attached_armies else "") + ", ".join(a['army_name'] for a in attached_armies)
                attacking_troops = army_info['troops'] + (sum(a['troops'] for a in attached_armues) if attached_armies else 0)
                ...
                battle_embed.add_field(name="Attacking Army",
                                       value=f"{attacking_armies}\n{attacking_troops:,.0f} troops")
            ...
            # for rebellions/civil wars/revolutions
                ...
                # === DEFEAT ===
                    ...
                    # get troop totals
                    troop_total = army_info['troops'] + (
                        sum(a['troops'] for a in attached_armies) if attached_armies else 0)
                    # check if the attacker army was destroyed or should be destroyed
                    if attacker_casualties >= (.8 * troop_total):
                        # destroy the army
                        await conn.execute('''DELETE
                                           ...
                                           army_info['army_id'])
                        # unassign any generals
                        await conn.execute('''UPDATE cnc_generals
                                              SET army_id = NULL
                                              WHERE army_id = $1
                                                 OR attached = $1;''',
                                           army_info['army_id'])
                    ...
                    # otherwise, normal defeat
                        ...
                        # retreat the attackers to whence they came
                        await conn.execute('''UPDATE cnc_armies SET location = $2 
                                              WHERE army_id = $1 OR attached = $1;''',
                                           army_info['army_id'], depart_prov_info['id'])
                        victor_string = "The rebels are victorious!"
    ...
    async def war_info(self, interaction: discord.Interaction, war_id: str):
        ...
        if (user_info['name'] == war_info['primary_attacker']) or (user_info['name'] == war_info['primary_defender']):
            ...
            if alliance_check:
                ...
                if user_info['name'] in war_info['attackers'] and not set(alliance_check['members']).difference(set(war_info['attackers'])):
                    alliance_button = False
                elif user_info['name'] in war_info['defenders'] and not set(alliance_check['members']).difference(set(war_info['defenders'])):
                    alliance_button = False
