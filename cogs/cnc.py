...
class CNC(commands.Cog):
    ...
    async def war_autocomplete(self, interaction: discord.Interaction, war_typed: str) -> List[discord.Choice(str)]:
        ...
        # construct list
        return [app_commands.Choice(name=f"{war['name']} (ID: {war['id']})", value=war['id']) for war in active_wars if (war_typed.lower() in war['name'].lower()) or (war_typed in str(war['id']))][0:24]
