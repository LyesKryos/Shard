...
class Verification(commands.Cog):
    ...
    async def verify(self, ctx):
        ...
        if nation_exist.status_code == 404:
            await author.send(f"No such nation as `{nation_name}`. Please check that you are using only the nation's"
                              f" name, without the pretitle.")
            return
        # send verification instructions via DM
        await author.send(f"Please login to {nation_name}. Once complete, head to this link and send the "
                ...
                if int(verification) == 1:
                    ...
                    if nation is None:
                        ...
                        if nation_name in nation_check:
                            await author.send("You have already verified that nation. To view a list of your verified"
                                              " nations, use `$view_verified`.")
                            return
