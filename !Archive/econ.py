...
class Economy(commands.Cog):
    ...
    def __init__(self, bot: Shard):
        ...
        self.market_task = self.bot.loop.create_task(self.market_updating(), name="market_updating")
        self.bank_task = self.bot.loop.create_task(self.bank_updating(), name="bank_updating")
        self.crash = False
        # define space
        self.space = "\u200b"
        # define logo
    ...
    async def bank_updating(self):
        ...
        while True:
            try:
                ...
                logger.exception("Bank Updating Error")
    ...
    async def market_updating(self):
        ...
        while True:
            try:
                ...
                for stock in stocks:
                    # define the number of new shares
                    new_shares = 0
                    # define trending
                    trending = "up"
                    # define value
                ...
                for u in users:
                    ...
                    member = thegye.get_member(user_id)
                ...
                logger.exception("Market updating error")
                await asyncio.sleep(5)
    ...
    @commands.command()
    @commands.is_owner()
    async def check_dead_users(self, ctx):
        # connect to pool
        conn = self.bot.pool
        # remove users no longer in the server
        users = await conn.fetch('''SELECT * FROM rbt_users;''')
