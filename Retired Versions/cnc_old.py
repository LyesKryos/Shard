...
class CNC(commands.Cog):
    ...
    version = "version 3.0 Golden Crowns"
    ...
    @cnc.command(name="info", description="Displays information about the CNC system.")
    @app_commands.guild_only()
    async def cnc_info(self, interaction: discord.Interaction):
        # defer interaction
        ...
        infoembed.add_field(name="About",
                            value="The Command and Conquest system is a simulated battle royale between "
                                  "the various nations players create. The system makes used of combat "
        ...
        infoembed.add_field(name="Questions?",
                            value="Contact the creator: lieskryos\n"
                                  "Contact a moderator: lieskryos, insert_person_here")
        infoembed.set_thumbnail(url="https://i.ibb.co/gTpHmgq/Command-Conquest-symbol.png")
        # send embed
        await interaction.followup.send(embed=infoembed)

    @cnc.command(name="turn", description="Displays the turn count.")
    @app_commands.guild_only()
    async def cnc_turn(self, interaction: discord.Interaction):
        # defer interaction
        await interaction.response.defer(thinking=False)
        ...
        conn = self.bot.pool
        data = await conn.fetchrow('''SELECT data_value FROM cnc_data WHERE data_name = $1;''', "turn")
        await interaction.followup.send(f"It is currently turn #{data['data_value']}. "
    ...
    @app_commands.describe(nation_name="The name of your new nation.",
                           color="The hex code of the color you want your nation to appear as.",
                           focus="The national focus of your nation.")
    @app_commands.guild_only()
    async def cnc_register(self, interaction: discord.Interaction, nation_name: str, color: str,
                           focus: Literal["Economic", "Strategic", "Military"]):
        # defer interaction
        ...
        if registered is not None:
            await interaction.followup.send(f"You are already registered as {registered['username']}.")
            return
        # checks the focus and ensures proper reading
        if focus == 'Military':
            research = ["Basic Metalworking"]
        elif focus.lower() == 'Economic':
            research = ["Currency"]
        else:
            research = ["Writing"]
        # if the color is banned, dont allow
        if color in self.banned_colors:
            await interaction.followup.send("That color is a reserved color. Please pick another color.")
            return
        ...
        if color_check is not None:
            await interaction.followup.send("That color is already taken by another user. Please pick another color.")
            return
        # try and get the color from the hex code
        try:
            ImageColor.getrgb(color)
        except ValueError:
            await interaction.followup.send("That doesn't appear to be a valid hex color code. Include the `#` symbol.")
            return
        ...
        await conn.execute(
            ...
            userid, nation_name, color, resources, focus.lower(), research, 0, 5)
        ...
        WHERE id = $3;''', userid, nation_name, province['id'])
        await self.bot.loop.run_in_executor(None, self.map_color, province['id'], province['cord'], color)
        tech = Technology(nation_name, ctx=ctx, techs=research)
        await tech.effects()
        await interaction.followup.send(f"{interaction.user.name} has registered {nation_name} in the Command and Conquer System\n"
                       f"{nation_name} has been given **\u03FE{resources}**, **3000 manpower**, and "
                       f"**Province #{province['id']}**. Use these gifts wisely!")

    @cnc.command(name="view", description="Displays information about a nation.")
    @app_commands.describe(user="The player you want to search for.", nation_name="The nation you want to search for.")
    @app_commands.guild_only()
    async def cnc_view(self, interaction: discord.Interaction, user: discord.User = None, nation_name: str = None):
        # defer interaction
        await interaction.response.defer(thinking=True, ephemeral=True)
        # connects to the database
        conn = self.bot.pool
        # if the nationame is left blank, the author id is used to find the nation information
        if (user is None) and (nation_name is None):
            ...
            cncuserembed.add_field(name="National Focus", value=userinfo['focus'])
            ...
            await interaction.followup.send(embed=cncuserembed)
        else:
            snowflake = False
            user = None
            try:
    ...
    async def cnc_unrest_score(self, ctx):
        ...
        WHERE owner_id = $1 and occupier_id = $1;''', ctx.author.id)
    ...
    async def cnc_change_rate(self, ctx, changed: str, rate: int):
        ...
        if changed == 'research' or changed == 'r':
            ...
            if current_rate == rate:
                await ctx.send(f"Your rate is already {rate}%!")
                return
            researching = await conn.fetchrow('''SELECT * FROM cnc_researching WHERE user_id = $1;''',
                                              author.id)
            if researching is not None:
                current_turns = researching['turns']
                reduction = (-rate/5) + 2
                previous_reduction = (-current_rate/5) + 2
                turns_added = reduction - previous_reduction
                current_turns += turns_added
                if current_turns < 0:
                    current_turns = 1
                await conn.execute('''UPDATE cnc_researching SET turns = $1 where user_id = $2;''',
                                   current_turns, userinfo['user_id'])
        ...
        while True:
            try:
                ...
                if str(reaction.emoji) == "\U0001fa99":
                    await map.clear_reactions()
                    await map.edit(content="https://i.ibb.co/TWH96nH/Trade-Goods-Map.png")
                    for react in reactions:
                        await map.add_reaction(react)
                    continue
                # close
